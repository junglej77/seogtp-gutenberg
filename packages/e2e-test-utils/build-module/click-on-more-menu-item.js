...
export async function clickOnMoreMenuItem(buttonLabel) {
  ...
  const moreMenuContainerSelector = '//*[contains(concat(" ", @class, " "), " interface-more-menu-dropdown__content ")]';
  const menuItems = await page.$x(`${moreMenuContainerSelector}//span[contains(concat(" ", @class, " "), " components-menu-item__item ")][contains(text(), "${buttonLabel}")]`);
