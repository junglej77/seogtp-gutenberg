...
export async function selectGlobalInserterTab(label) {
  ...
  const activeTabLabel = await page.evaluate(el => el.innerText, activeTab);
  ...
  const targetTab = await page.waitForXPath(`//div[contains(@class, "block-editor-inserter__tabs")]//button[${labelSelector}]`);
    ...
    case 'Synced patterns':
      {
        waitForInsertElement = async () => {
          return await page.waitForXPath(`//*[@role='option' and contains(., '${searchTerm}')]`);
        };
        waitForNoResults = async () => {
          await page.waitForSelector('.block-editor-inserter__no-results');
          return null;
        };
        break;
      }
    case 'Block Directory':
      {
        waitForInsertElement = async () => {
          ...
          return await page.waitForSelector('.block-directory-downloadable-blocks-list button:first-child');
        };
        waitForNoResults = async () => {
          ...
          return await new Promise(resolve => setTimeout(() => resolve(null), 5000));
        };
      }
  }
...
export async function insertFromGlobalInserter(category, searchTerm) {
  ...
  if (['Blocks', 'Synced patterns'].includes(category)) {
    ...
    try {
      insertButton = (await page.$x(`//*[@role='option' and contains(., '${searchTerm}')]`))[0];
    } catch (error) {
      // noop
    }
  }
  ...
  if (!insertButton) {
    throw new Error(`Couldn't find "${searchTerm}" in the ${category} category.`);
  }
  ...
  if (category === 'Synced patterns') {
    await canvas().waitForSelector('.block-library-block__reusable-block-container');
  }

  // Extra wait for the Block Directory block to be ready.
  if (category === 'Block Directory') {
    await page.waitForSelector('.block-directory-downloadable-blocks-list button:first-child:not(.is-busy)');
  }
