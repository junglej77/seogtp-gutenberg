...
export async function deleteAllMenus() {
  const menus = await rest({
    path: menusEndpoint
  });
  await batch(menus.map(menu => ({
    method: 'DELETE',
...
export async function createMenu(menu, menuItems) {
  ...
  const objectRequests = menuItems.map(menuItem => {
  ...
  const menuItemsResponse = await batch(menuItems.map(menuItem => {
    // If the menu item is linked to an 'object', get the id for that
    // object.
    const objectMatcher = menuItemObjectMatchers[menuItem.object];
    let object;
    if (objectMatcher) {
      object = objectResponses.find(objectResponse => objectMatcher(menuItem, objectResponse));
    }
    return {
      method: 'POST',
  ...
  await batch(menuItems.map((menuItem, index) => {
    ...
    const parent = menuItemsResponse.responses[menuItem.parent].body.id;
