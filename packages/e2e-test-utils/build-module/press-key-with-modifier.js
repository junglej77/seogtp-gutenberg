...
async function emulateSelectAll() {
  await page.evaluate(() => {
    ...
    const canvasDoc = (_document$activeEleme = document.activeElement.contentDocument) !== null && _document$activeEleme !== void 0 ? _document$activeEleme : document;
    canvasDoc.activeElement.dispatchEvent(new KeyboardEvent('keydown', {
      bubbles: true,
      cancelable: true,
    ...
    canvasDoc.activeElement.dispatchEvent(new KeyboardEvent('keyup', {
      bubbles: true,
      cancelable: true,
      key: isMac ? 'Meta' : 'Control',
      code: isMac ? 'MetaLeft' : 'ControlLeft',
...
export async function setClipboardData({
  ...
  await page.evaluate((_plainText, _html) => {
...
async function emulateClipboard(type) {
  await page.evaluate(_type => {
    ...
    const canvasDoc = (_document$activeEleme2 = document.activeElement.contentDocument) !== null && _document$activeEleme2 !== void 0 ? _document$activeEleme2 : document;
    if (_type !== 'paste') {
      window._clipboardData = new DataTransfer();
    ...
    canvasDoc.activeElement.dispatchEvent(new ClipboardEvent(_type, {
      bubbles: true,
      cancelable: true,
...
export async function pressKeyWithModifier(modifier, key) {
  ...
  await Promise.all(mappedModifiers.map(async mod => {
