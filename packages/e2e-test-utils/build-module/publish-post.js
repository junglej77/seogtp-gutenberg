...
export async function publishPost() {
  const publishPanelToggle = await page.$('.editor-post-publish-panel__toggle');
  const isPublishingPost = await publishPanelToggle.evaluate(element => element.textContent === 'Publish');
  const isEntityPublishToggle = await publishPanelToggle.evaluate(element => element.classList.contains('has-changes-dot'));
  ...
  if (isPublishingPost) {
    ...
    const publishButton = await page.waitForSelector('.editor-post-publish-button:not([aria-disabled="true"])');
