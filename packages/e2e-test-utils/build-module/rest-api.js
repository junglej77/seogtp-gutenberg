...
async function login(retries = 3) {
  ...
  apiFetch.nonceEndpoint = createURL('wp-admin/admin-ajax.php', 'action=rest-nonce');
  ...
  throw new Error(`Fetch api call failed for ${apiFetch.nonceEndpoint}: ${response.status}`);
}
