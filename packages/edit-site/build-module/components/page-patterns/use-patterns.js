...
import { EXCLUDED_PATTERN_SOURCES, PATTERN_TYPES, PATTERN_SYNC_TYPES, TEMPLATE_PART_POST_TYPE, TEMPLATE_PART_AREA_DEFAULT_CATEGORY } from '../../utils/constants';
import { unlock } from '../../lock-unlock';
import { searchItems } from './search-items';
import { store as editSiteStore } from '../../store';
const EMPTY_PATTERN_LIST = [];
const selectTemplateParts = createSelector((select, categoryId, search = '') => {
...
}, select => [select(coreStore).getEntityRecords('postType', TEMPLATE_PART_POST_TYPE, {
  per_page: -1
}), select(coreStore).isResolving('getEntityRecords', ['postType', TEMPLATE_PART_POST_TYPE, {
  per_page: -1
}]), select(editorStore).__experimentalGetDefaultTemplatePartAreas()]);
const selectThemePatterns = createSelector(select => {
...
}, select => [select(coreStore).getBlockPatterns(), select(coreStore).isResolving('getBlockPatterns'), unlock(select(editSiteStore)).getSettings()]);
const selectPatterns = createSelector((select, categoryId, syncStatus, search = '') => {
  const {
...
const selectUserPatterns = createSelector((select, syncStatus, search = '') => {
  const {
    getEntityRecords,
    isResolving: isResolvingSelector,
...
}, select => [select(coreStore).getEntityRecords('postType', PATTERN_TYPES.user, {
  per_page: -1
}), select(coreStore).isResolving('getEntityRecords', ['postType', PATTERN_TYPES.user, {
  per_page: -1
...
export const usePatterns = (postType, categoryId, {
  search = '',
  syncStatus
} = {}) => {
