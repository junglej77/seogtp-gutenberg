...
export default function StyleVariationsContainer( { gap = 2 } ) {
	const { user } = useContext( GlobalStylesContext );// 从GlobalStylesContext中获取当前用户信息
	...
	const { setCurrentStyle } = useDispatch('seogtp-gutenberg/seogtp_style_store'); // seogtp获取更新 currentStyle 的 di
	...
	const handleAddStyle = async () => {
        ...
        const styleExists = themeVariations.some(
            (variation) => variation.title.toLowerCase() === trimmedStyleName.toLowerCase()
        );
		if (styleExists) {
            alert(__('已经创建此风格'));
            ...
            try {
                ...
                const createResponse = await apiFetch({
                    ...
                    method: 'POST',
                    data: { new_style_name: trimmedStyleName },
                });
				// 使缓存失效，确保全局样式的重新获取
	...
	return (
		...
		>
			{ themeVariations.map( ( variation, index ) => (
				<Variation key={ index } variation={ variation } currentStyle={currentStyle}>
					{ ( isFocused ) => (
						<PreviewStyles
		...
		{isModalOpen && (
                ...
                >
                    ...
                    />
                    <Button isPrimary onClick={handleAddStyle}>
                        {__('确定')}
                    </Button>
