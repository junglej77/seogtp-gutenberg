 ...
 */
import { findNodeHandle, requireNativeComponent, UIManager, Pressable, Platform, TouchableWithoutFeedback } from 'react-native';
...
class AztecView extends Component {
  ...
  dispatch(command, params) {
    params = params || [];
    UIManager.dispatchViewManagerCommand(this.aztecViewRef.current, command, params);
  }
  ...
  _onChange(event) {
    ...
    if (Platform.OS === 'android') {
      const triggersIncludeEventKeyCode = this.props.triggerKeyCodes && this.props.triggerKeyCodes.map(char => char.charCodeAt(0)).includes(event.nativeEvent.keyCode);
  ...
  updateCaretData(event) {
    if (this.isFocused() && this.selectionEndCaretY !== event?.nativeEvent?.selectionEndCaretY) {
  ...
  onRemoveMarkFormatting() {
    UIManager.dispatchViewManagerCommand(findNodeHandle(this.aztecViewRef.current), 'onRemoveMarkFormatting', []);
  }
  onMarkFormatting(color) {
    UIManager.dispatchViewManagerCommand(findNodeHandle(this.aztecViewRef.current), 'onMarkFormatting', [color]);
  }
  _onPress(event) {
    if (!this.isFocused()) {
  ...
  render() {
    ...
    if (Platform.OS === 'android' && this.props.text?.tag === 'pre' && style.hasOwnProperty('fontSize')) {
    ...
    const TouchableComponent = Platform.OS === 'ios' ? Pressable : TouchableWithoutFeedback;
    return /*#__PURE__*/_jsx(TouchableComponent, {
      accessible: Platform.OS !== 'ios',
      onPress: this._onPress,
      children: /*#__PURE__*/_jsx(RCTAztecView, {
        ...otherProps,
        style: style,
...
//# sourceMappingURL=AztecView.js.map