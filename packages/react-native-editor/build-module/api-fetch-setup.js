...
const SUPPORTED_ENDPOINTS = {
  ...
  GET: [/wp\/v2\/(media|categories|blocks)\/?\d*?.*/i, /wp\/v2\/search\?.*/i, /oembed\/1\.0\/proxy\?.*/i],
  POST: []
};

// [ONLY ON ANDROID] The requests made to these endpoints won't be cached.
const DISABLED_CACHING_ENDPOINTS = [/wp\/v2\/(blocks)\/?\d*?.*/i, /oembed\/1\.0\/proxy\?.*/i];
...
const fetchHandler = ({
  ...
  data,
  parse
}, retries = 20, retryCount = 1) => {
  if (!isMethodSupported(method)) {
    return Promise.reject(`Unsupported method: ${method}`);
  }
  if (!isPathSupported(path, method)) {
    return Promise.reject(`Unsupported path for method ${method}: ${path}`);
  }
  let responsePromise;
  switch (method) {
    case 'GET':
  ...
  const parseResponse = response => {
    ...
    if (parse === false) {
      const body = isStringResponse ? response : JSON.stringify(response);
  ...
  return responsePromise.then(parseResponse).catch(error => {
    ...
    return setTimeoutPromise(1000 * retryCount).then(() => fetchHandler({
      path
    }, retries - 1, retryCount + 1));
  });
};
export const isMethodSupported = method => SUPPORTED_METHODS.includes(method);
export const isPathSupported = (path, method) => {
  const supportedEndpoints = applyFilters('native.supported_endpoints', SUPPORTED_ENDPOINTS);
  return supportedEndpoints[method].some(pattern => pattern.test(path));
};
export const shouldEnableCaching = path => {
  const disabledEndpoints = applyFilters('native.disabled_caching_endpoints', DISABLED_CACHING_ENDPOINTS);
  return !disabledEndpoints.some(pattern => pattern.test(path));
};
export default (() => {
  apiFetch.setFetchHandler(options => fetchHandler(options));
});
//# sourceMappingURL=api-fetch-setup.js.map