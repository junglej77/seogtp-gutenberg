...
const reactNativeSetup = () => {
  LogBox.ignoreLogs(['Require cycle:' // TODO: Refactor to remove require cycles
  ]);
...
const gutenbergSetup = () => {
  ...
  wpData.use(wpData.plugins.persistence, {
    storageKey
  });
  setupApiFetch();
  setupInitHooks();
...
const setupInitHooks = () => {
  addAction('native.pre-render', 'core/react-native-editor', props => {
    var _props$capabilities;
    const capabilities = (_props$capabilities = props.capabilities) !== null && _props$capabilities !== void 0 ? _props$capabilities : {};
    registerBlocks();

    // Unregister non-supported blocks by capabilities
    if (getBlockType('core/block') !== undefined && capabilities.reusableBlock !== true) {
  ...
  addFilter('native.block_editor_props', 'core/react-native-editor', props => {
    const {
      capabilities = {}
    } = props;
    let {
