...
const TEST_LOG_REGEX = new RegExp(`<time datetime="(?<date>.+)">.*</time>\\s*Test passed after (?<failedTimes>\\d) failed attempts? on <a href="(?<runURL>.+)"><code>(?<headBranch>.+)</code></a>\\.`);
const TEST_FULL_REGEX = new RegExp(`(?:<details>\\s*<summary>\\s*)?${TEST_LOG_REGEX.source}(?:\\s*</summary>\\s*
\`\`\`
(?<errorMessage>[\\s\\S]*)
\`\`\`
\\s*</details>)?`);
    ...
    case '@playwright/test':
      {
        ...
        return stripAnsi(flakyTestResult.results.map(result => result.error.stack).join('\n'));
      }
    case 'jest-circus':
    default:
      {
        return stripAnsi(formatResultsErrors(flakyTestResult.results, {
...
function parseIssueBody(body) {
  ...
  const testResults = Array.from(body.matchAll(new RegExp(`^${TEST_RESULT.open}[\\s\\S]+?${TEST_RESULT.close}$`, 'gm'))).map(([match]) => match).map(testResult => {
    try {
