...
import { renderIssueBody, formatTestErrorMessage, formatTestResults, parseIssueBody, renderCommitComment, isReportComment } from './markdown';
async function run() {
  ...
  const flakyTests = await Promise.all(flakyTestsDir.map(filename => fs.readFile(path.join(artifactPath, filename), 'utf-8').then(text => JSON.parse(text))));
  ...
  github.context.payload.pull_request.head.ref : ref.replace(/^refs\/(heads|tag)\//, '');
  ...
  for (const flakyTest of flakyTests) {
    ...
    const reportedIssue = issues.find(issue => issue.title === issueTitle);
    ...
    if (reportedIssue) {
      ...
      if (reportedIssue.closed_at) {
        try {
          ...
          const latestAncestorCommit = await api.findMergeBaseCommit('trunk', github.context.sha);
      ...
      const {
        meta,
        testResults: prevTestResults
      } = parseIssueBody(body);

      // Concat the test results list with the latest test results.
      const formattedTestResults = [...prevTestResults.map(testResult => formatTestResults({
        ...testResult,
        // Don't output previous test results' stacks.
        errorMessage: undefined
      })), currentFormattedTestResults].join('\n<br/>\n');
  ...
  const {
    html_url: commentUrl
  } = isPR ? await api.createCommentOnPR(
  // Cast the payload type: https://github.com/actions/toolkit/tree/main/packages/github#webhook-payload-typescript-definitions
  github.context.payload.number, renderCommitComment({
    runURL,
    reportedIssues,
    commitSHA: github.context.payload.pull_request.head.sha
  }), isReportComment) : await api.createCommentOnCommit(github.context.sha, renderCommitComment({
    runURL,
    reportedIssues,
